...
const FormVisaCanada = ({ filename, setPhotos,fname,setFname,selectedOption,setSelectedOption}) => {
  ...
  const HandleBeginButton = () => {
    setModalShow(true);
    PhotoService.getPhotos(filename, "can@vis",selectedOption, count).then(
      (response) => {
    ...
    <>
      {!showphotos && (
        <Container className="d-flex justify-content-center align-items-center min-vh-20">
          ...
          >
            <Card.Body>
              <h1 className="mb-1">Foto Visa para Canada</h1>
              <h4 className="mb-4">(35mm x 45mm)</h4>
              <Row>
                <Col md={6}>
                  <Image
                    src={image}
